...
function buildEvidence(type, value, source = 'deep-check') {
    return {
        type,
        value,
        source
...
function addScore(state, bucket, score, reason, evidence) {
...
function applyEntrypoints(state, entrypoints) {
...
function applyMixins(state, mixins) {
...
function applyArchiveContent(state, archiveContent) {
...
function resolveScoredDecision(state, triggerReasons, analysis) {
    if (state.keepScore >= 4 && state.keepScore >= state.removeScore + 2) {
        return createResult({
            ...
            triggerReasons,
            warnings: analysis.mixins.warnings,
            errors: [],
            analysis
        });
    }
...
function resolveBuildDecision(resolvedDecision, profile, currentBuildDecision) {
...
function performDeepCheck(input, triggerReasons = []) {
...
function applyDeepCheckResultToDecision(decision, result, profile) {
    ...
    if (result.status === DEEP_CHECK_STATUSES.failed) {
        return {
            ...
            finalReasons: result.reasons,
            decision: profile === ARBITER_PROFILES.aggressive ? 'keep' : 'keep',
            reason: result.reason,
