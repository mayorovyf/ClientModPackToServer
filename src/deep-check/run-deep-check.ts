...
import type { DeepCheckMode, DeepCheckResult, DeepCheckSummary, DeepCheckTrigger } from '../types/deep-check';
import type { RunContext } from '../types/run';

function buildDeepCheckStats(decisions: Array<Record<string, any>>, mode: DeepCheckMode): DeepCheckSummary {
    const summary = createEmptyDeepCheckSummary(mode);

    for (const decision of decisions) {
        const deepCheck = decision.deepCheck as DeepCheckResult | null;
...
function createSkippedDeepCheck(trigger: DeepCheckTrigger): DeepCheckResult {
    return {
...
function runDeepCheck({
    ...
    return {
        ...
        report: {
            ...
            summary: buildDeepCheckStats(updatedDecisions, runContext.deepCheckMode as DeepCheckMode),
            errors: []
        }
    };
}
