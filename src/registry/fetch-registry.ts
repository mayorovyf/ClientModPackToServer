...
async function fetchWithTimeout(
    url: string,
    {
        timeoutMs = DEFAULT_REGISTRY_FETCH_TIMEOUT_MS,
        headers = {}
...
async function fetchRegistryText(url: string, options: FetchWithTimeoutOptions = {}): Promise<string> {
    const response = await fetchWithTimeout(url, options);
    return response.text();
}

async function fetchRegistryBuffer(url: string, options: FetchWithTimeoutOptions = {}): Promise<Buffer> {
    const response = await fetchWithTimeout(url, options);
