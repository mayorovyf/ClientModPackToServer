...
function chooseBestResult(results = [], decision) {
    return results
...
function listEngineNames(results = [], decisionFilter = null) {
    return results
...
function extractSignalState(input) {
...
function extractDependencyState(input) {
    const preservedByDependency = input.dependencyAdjusted || input.dependencyFindings.some((finding) => finding.type === 'preserved-by-dependency');
...
function createWinningEvidence(source) {
    if (!source) {
        return [];
    }
...
function createFindingEvidence(findings) {
...
function maybeEscalateRemoveByProfile(profile, signal) {
    if (!signal) {
        return false;
    }
...
function createConflictPayload(signalState) {
    return {
...
function arbitrateDecision(input) {
...
function buildArbiterStats(decisions, profile) {
    const summary = {
        profile,
        finalDecisions: {
            keep: 0,
