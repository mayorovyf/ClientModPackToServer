...
import type { ClassificationEngine, EngineDecision, PickedRegistryRule, ScoredRegistryRule } from '../../types/classification';
import type { ModDescriptor } from '../../types/descriptor';
import type { RegistryRule } from '../../types/registry';

function collectDescriptorIds(descriptor: ModDescriptor): Set<string> {
    return new Set(
...
function scoreRuleMatch(rule: RegistryRule, descriptor: ModDescriptor, descriptorIds: Set<string>): number {
...
function pickBestRule(descriptor: ModDescriptor, registry: { rules: RegistryRule[] }): PickedRegistryRule {
    ...
    const scored = registry.rules
        ...
        .filter((item): item is ScoredRegistryRule => item.score > 0)
        .sort((left, right) => {
...
function sideToDecision(side: RegistryRule['side']): EngineDecision {
    if (side === 'client') {
...
const registryEngine: ClassificationEngine = {
