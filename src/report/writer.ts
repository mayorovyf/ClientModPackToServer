...
function createSummary(report: RunReport): string {
    ...
    const lines = [
        ...
        `- Cache hit: ${report.serverCoreInstall ? (report.serverCoreInstall.cacheHit ? 'yes' : 'no') : 'n/a'}`,
        '',
        '## Workspace',
        '',
        `- Materialization mode: ${report.workspace ? report.workspace.materializationMode : 'n/a'}`,
        `- Manifest path: ${report.workspace ? (report.workspace.manifestPath || 'n/a') : 'n/a'}`,
        `- Stash mods dir: ${report.workspace ? (report.workspace.stashModsDir || 'n/a') : 'n/a'}`,
        `- Active mods: ${report.workspace ? report.workspace.currentActiveMods : 0}`,
        `- Delta keep files: ${report.workspace?.delta ? report.workspace.delta.desiredKeepFiles.length : 0}`,
        `- Delta exclude files: ${report.workspace?.delta ? report.workspace.delta.desiredExcludeFiles.length : 0}`,
        `- Mods to add: ${report.workspace?.delta ? report.workspace.delta.modsToAdd.length : 0}`,
        `- Mods to reuse: ${report.workspace?.delta ? report.workspace.delta.modsToReuse.length : 0}`,
        `- Mods restored from stash: ${report.workspace?.delta ? report.workspace.delta.modsToRestoreFromStash.length : 0}`,
        `- Mods moved to stash: ${report.workspace?.delta ? report.workspace.delta.modsToStash.length : 0}`,
        `- Materialization copied: ${report.workspace?.materialization ? report.workspace.materialization.copied : 0}`,
        `- Materialization linked: ${report.workspace?.materialization ? report.workspace.materialization.linked : 0}`,
        `- Materialization reused: ${report.workspace?.materialization ? report.workspace.materialization.reused : 0}`,
        `- Materialization restored: ${report.workspace?.materialization ? report.workspace.materialization.restoredFromStash : 0}`,
        `- Materialization stashed: ${report.workspace?.materialization ? report.workspace.materialization.movedToStash : 0}`,
        `- Materialization removed stale: ${report.workspace?.materialization ? report.workspace.materialization.removedStale : 0}`,
        `- Skeleton entries copied: ${report.workspace?.materialization ? report.workspace.materialization.skeletonEntriesCopied : 0}`,
        `- Validation-only materialization: ${report.workspace?.materialization ? (report.workspace.materialization.validationOnly ? 'yes' : 'no') : 'n/a'}`,
        `- Validation sandbox linked files: ${report.workspace?.validationSandbox ? report.workspace.validationSandbox.linkedFiles : 0}`,
        `- Validation sandbox copied files: ${report.workspace?.validationSandbox ? report.workspace.validationSandbox.copiedFiles : 0}`,
        `- Validation sandbox copied directories: ${report.workspace?.validationSandbox ? report.workspace.validationSandbox.copiedDirectories : 0}`,
        '',
        '## Stats',
        '',
        ...
        '## Joinability',
        '',
        `- Status: ${report.validation ? (report.validation.joinability?.status || 'not-checked') : 'n/a'}`,
        `- Checked by: ${report.validation ? ((report.validation.joinability?.checkedBy || []).join(', ') || 'n/a') : 'n/a'}`,
        `- Success markers: ${report.validation ? (report.validation.joinability?.successMarkers.length || 0) : 0}`,
        `- Failure markers: ${report.validation ? (report.validation.joinability?.failureMarkers.length || 0) : 0}`,
        `- Evidence: ${report.validation ? (report.validation.joinability?.evidence.join(' | ') || 'n/a') : 'n/a'}`,
        '',
        '## Failure Analysis',
        '',
